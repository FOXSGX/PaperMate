...
PaperMate 旨在构建一个覆盖学术写作主要环节、可演示并可持续扩展的智能辅助平台，具体目标如下：

1. **打通学术写作工作流**：在一个应用中完成文献检索、综述生成、论文上传、文档问答、格式排版和文本改写。
2. **降低资料整理成本**：通过结构化综述、文档切片检索和来源引用，减少人工检索、阅读与归纳的重复工作。
3. **保证离线演示能力**：未配置 LLM API Key 时仍可使用规则与模板完成核心链路，避免外部服务不可用导致作品无法演示。
4. **支持能力按需替换**：保持前后端 API 契约稳定，使检索、RAG、生成和排版模块能够逐步替换为真实 LLM、向量数据库及更多文献数据源。
5. **达到可验证的工程基线**：确保项目能够本地启动、完成生产构建，并通过后端与前端自动化测试；后续继续补充 E2E、生产部署和安全能力。
...
- Python 3.10+
- Node.js 18+
- （可选）Docker / Docker Compose — 用于前后端开发编排；本地开发不依赖 Docker
...
| **角色B：前端 & 应用** | 补齐任务状态、失败重试和进度展示；完善最近上传入口；补 Playwright E2E 测试 | 前端体验仍是演示级；工程化与测试覆盖仍需加强 |
| **角色C：文档处理** | 增强 PDF/DOCX 解析对双栏、表格、公式和扫描件的支持；完善 DOCX 参考文献、目录、图表编号和脚注排版；引入 OCR、版面分析与 CSL 样式支持 | 文档处理能力不完整；RAG 切片质量有限 |
...
- [x] DOCX 格式排版引擎与样式配置
- [x] 规则化降重改写
- [x] 前端六个主视图：首页、上传、文档管理、问答、综述、排版降重
- [x] Vite 开发代理与基础 UI（Tailwind）
- [x] 综述大纲风格（标准、方法导向、时间线）前后端联动
- [x] 文档列表、删除、快速进入问答与本地会话历史
- [x] 文档处理模块单测与技术说明文档
- [x] 后端测试套件（40 项）与前端组件测试（44 项）
- [x] 本地前后端启动、健康检查和 Vite 代理联调验证
...
- 文档管理页已支持列表、删除、快速进入问答和本地会话历史，但还缺少独立的最近上传入口、跨文档会话总览和更清晰的批量管理能力。

**改进方向**：补充任务状态 API、文档管理页、会话历史、失败重试和更明确的进度反馈。

### 6. 工程化与测试覆盖仍需加强

- 后端已有 40 项 pytest，前端已有 44 项组件测试，但 API 层、异常路径、前后端联调和 SSE 流式接口的自动化测试仍不够完整。
- 前端尚缺 Playwright 端到端测试，难以及时发现跨页面交互回归。
